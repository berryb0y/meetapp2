...
describe('<Event /> component', () => {
    ...
    test('show correct event title', () => {
        expect(EventWrapper.find('.title').text()).toEqual(mockData[1].summary);
    });
    test('show correct start-time', () => {
        const mockDataDateFormat = `${new Date(mockData[1].start.dateTime)}`;
        expect(EventWrapper.find('.start-time').text()).toEqual(
          `${mockDataDateFormat}`
        );
    });
    test('show correct event location', () => {
        expect(EventWrapper.find('.location').text()).toEqual(mockData[1].location);
    });
